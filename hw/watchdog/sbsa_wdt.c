...
REG32(SBSA_GWDT_W_IIDR, 0xfcc)
REG32(SBSA_GWDT_IDR, 0xfd0)

static inline void log_watchdog(const char *name, uint64_t offset, uint64_t data, unsigned size)
{
    struct timeval _now;

    if (0) {
	gettimeofday(&_now, NULL);
	qemu_log("%d@%zu.%06zu:%s: offset 0x%" PRIx64 " data 0x%" PRIx64 " size %u" "\n",
             qemu_get_thread_id(),
...
static void sbsa_watchdog_update(SBSAWatchdog *s)
{
    if (!s->enable)
	    return;

    /* TBD: add interrupt support
     * if (ptimer_get_limit(s->timer)/2 < s->cur_tick)
					   ...
					   unsigned size)
{
    ...
    */
    case A_SBSA_GWDT_WOR:
        r = ptimer_get_count(s->timer); /* time left */
        break;
